...
- 수동 공고 추가: `settings.py:MANUAL_JOBS_BY_REGION`의 해당 지역 목록에 정상 항목을 복사해 수정한다.
  위에 나열한 13개 필드를 정확히 유지하고 값은 모두 문자열로 쓴다. 선택 정보가 없으면 필드를
  지우거나 `None`으로 바꾸지 말고 `''`를 사용한다. 실제 내용은 기존 파이프라인 필터도 통과해야 한다.
- 수동 공고 비활성화: 공고 딕셔너리 전체를 제거하거나 전체 블록을 주석 처리한다.
  일부 필드만 주석 처리하면 스키마가 깨질 수 있다. 필드 오타는 항목 생성 오류를 일으킬 수 있고,
  최종 결과에 남은 필드 누락·비문자열 값은 전체 반영을 막는다. 앞선 필터에서 제외된 항목은 다르다.
- 수동 공고는 `get_job_detail`의 정제를 거치지 않고 파이프라인에 들어가므로 표시할 문자열을 직접 쓴다.
- 차단 번호·제외 규칙 추가: `pipelines.py:Recru_It_Pipeline.process_item`의 해당 기존 규칙을 따른다.
  급여 정책을 바꾸면 `pacing.py:salary_exclusion`과 스파이더의 표본 검사도 함께 대조한다.
  `pattern_day10_14`, `pattern_month10_14`는 스파이더가 import해 상세 표본 검사에 쓰므로 단독으로 삭제하거나 이름을 바꾸지 않는다.
- 텍스트 정제 추가: 스파이더의 `get_job_detail(raw)`에서 검증된 값을 정제한다. 화면을 다시 읽지 않는다.
  급여·연락처·제목 변경은 표본 검사·제외·중복 판단에도 영향을 줄 수 있으므로 관련 결과를 확인한다.
